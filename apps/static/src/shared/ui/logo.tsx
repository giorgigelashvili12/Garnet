...
export default function Logo({ className = "", hideText = false }: LogoProps) {
    const content = (
        <div className={`flex items-center gap-3 group text-slate-900 dark:text-white ${className}`}>
            <Image src="/crystal-1.png" alt="Logo" width={40} height={40} className="transition-transform group-hover:scale-110" />
            {!hideText && <span className="font-black tracking-tighter text-2xl">Garnet</span>}
        </div>
    );
