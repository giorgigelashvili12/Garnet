...
export default function HardwareSection() {
  ...
  const updateArrows = useCallback(() => {
    ...
    if (!t) return;
    setCanPrev(t.scrollLeft > 10);
    setCanNext(t.scrollLeft < t.scrollWidth - t.clientWidth - 10);
  }, []);

  const scroll = useCallback((dir: "prev" | "next") => {
    const t = trackRef.current;
    if (!t) return;
    const scrollAmount = t.clientWidth * 0.8;
    t.scrollBy({ left: dir === "next" ? scrollAmount : -scrollAmount, behavior: "smooth" });
  }, []);
